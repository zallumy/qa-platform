...
CREATE TABLE organizations (
    ...
    settings        JSONB NOT NULL DEFAULT '{}',  -- e.g. custom DPI thresholds per org
...
CREATE TABLE files (
    ...
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    org_id          UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    storage_key     TEXT NOT NULL,          -- path/key in S3 or MinIO
...
CREATE TABLE analysis_reports (
    ...
    file_format         VARCHAR(20),         -- PDF / PNG / JPEG / TIFF
    width_px            INT,
    height_px           INT,
    ...
    -- rich data
    fonts               JSONB NOT NULL DEFAULT '[]',   -- [{name, embedded: bool}]
    color_palette        JSONB NOT NULL DEFAULT '[]',   -- [{hex, coverage_pct}]

    overall_pass        BOOLEAN,
    pdf_report_key      TEXT,                -- generated PDF report in storage
...
CREATE TABLE pantone_matches (
    ...
    pantone_code    VARCHAR(30),             -- null if using open approximation without official codes
    reference_label VARCHAR(100),            -- "closest color reference" when unlicensed
    delta_e         NUMERIC(6,3),            -- CIEDE2000 distance
...
CREATE TABLE logos (
    ...
    reasons             JSONB NOT NULL DEFAULT '[]',   -- ["DPI below 300", "No transparent background"]
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE audit_log (
    ...
    action          VARCHAR(100) NOT NULL,     -- e.g. "user.update_role", "org.update_settings"
    target_type     VARCHAR(50) NOT NULL,      -- "user" | "file" | "organization" | "report"
    target_id       UUID,
